...
Partial Class Herramientas_ConsultaOperaciones
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            oper.GenerarSeguimientoActividadUsuario(Session("IdUsuario"), "Ingreso a Consulta: Operaciones", "Consulta Libro de Órdenes", "")
            ...
            Dim FechaOperacion As Date = oper.ExecuteScalar("Select Top(1) Fecha from [vSubasta] order by Fecha desc")
        ...
        If txtNombreConsultaUsuario.Text <> "" Then
            Page.Header.Title = "Consulta de Libro de Órdenes -> " & txtNombreConsultaUsuario.Text
        End If
    ...
    Protected Sub RadToolBar1_ButtonClick(sender As Object, e As RadToolBarEventArgs) Handles RadToolBar1.ButtonClick, RadToolBar2.ButtonClick
        ...
        If e.Item.Value <> "" Then
            ...
            If e.Item.Value = 0 Then 'Mover
                ...
                InjectScriptLabelImprimir.Text = "<script>ventanaSecundaria('../Consultas/ConsultaLibroDeOrdenes.aspx','1000','600')</" + "script>"
            ...
            ElseIf e.Item.Value = 2 Then 'Guardar
                ...
                MyWindow.NavigateUrl = "NombreConsultaUsuario.aspx?ID=" & txtIdConsulta.Text & "&NC=ConsultaLibroDeOrdenes&U=" & txtIdUsuario.Text & "&NCU=" & txtNombreConsultaUsuario.Text '& "&F=" & RadFilter1.SaveSettings()
        ...
        Else
            ...
            If e.Item.Text <> "" Then
                ...
                Page.Header.Title = "Consulta Libros de Órdenes -> " & e.Item.Text
    ...
    Protected Sub CargarInformacionFiltro(cNombreConsultaUsuario As String)
        ...
        Dim ds As DataSet = oper.ExDataSet("SELECT idConsulta,Filtro, EstruturaGrid FROM  [vFiltrosConsultas]  where NombreConsultaUsuario='" & cNombreConsultaUsuario & "' AND IdUsuario='" & txtIdUsuario.Text & "' AND NombreConsulta='ConsultaLibroDeOrdenes'")
    ...
    Protected Sub LlenarInformacionListaFiltros()
        ...
        Dim ds As DataSet = oper.ExDataSet("SELECT NombreConsultaUsuario FROM  [vFiltrosConsultas]  where IdUsuario='" & txtIdUsuario.Text & "' AND NombreConsulta='ConsultaLibroDeOrdenes'")
        Dim MyRow As DataRow
        Dim cStringFiltro As String = ""
    ...
    Protected Sub LlenarInformacionListaFiltros1()
        ...
        Dim ds As DataSet = oper.ExDataSet("SELECT NombreConsultaUsuario FROM  [FiltrosConsultas]  where IdUsuario='" & txtIdUsuario.Text & "' AND NombreConsulta='ConsultaLibroDeOrdenes' ORDER BY idConsulta ASC")
        Dim MyRow As DataRow
        Dim cStringFiltro As String = ""
    ...
    Sub DataRefresh()
        ...
        SqlvOperacionesCSV.SelectCommand = "SP_ConsultaLibroDeOrdenes"
    End Sub
    ...
    'Protected Sub RadFilter1_PreRender(sender As Object, e As EventArgs) Handles RadFilter1.PreRender

    '    Try
    '        InjectScriptLabel.Text = "<script>applyExpression</" + "script>"
    '    Catch ex As Exception
    '    End Try
