...
Partial Class ConsultaOperacionesNotificacionesCevaldom
    Inherits Page
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            oper.GenerarSeguimientoActividadUsuario(Session("IdUsuario"), "Ingreso a Consulta: Notificación Operaciones", "Consulta de Notificaciones Operaciones Cevaldom", "")
            ...
            Dim FechaOperacion As Date = oper.ExecuteScalar("Select Top(1) fecha  from SIOPEL_INTERFACE_DB.dbo.V_OPERACIONES_ESTADO order by fecha desc")
            cStringWhere = String.Format("[fecha]='{0}'", FechaOperacion)
    ...
    Protected Sub RadToolBar1_ButtonClick(sender As Object, e As RadToolBarEventArgs) Handles RadToolBar1.ButtonClick, RadToolBar2.ButtonClick
        If e.Item.Value <> "" Then
            If e.Item.Value = 0 Then 'Mover
                InjectScriptLabelImprimir.Text = "<script>ventanaSecundaria('../Consultas/ConsultaOperacionesNotificacionesCevaldom.aspx','1000','600')</" + "script>"
            ...
            ElseIf e.Item.Value = 2 Then 'Guardar
                ...
                MyWindow.NavigateUrl = String.Format("NombreConsultaUsuario.aspx?ID={0}&NC=ConsultaOperacionesNotificacionesCevaldom&U={1}&NCU={2}", txtIdConsulta.Text, txtIdUsuario.Text, txtNombreConsultaUsuario.Text) '& "&F=" & RadFilter1.SaveSettings()
        ...
        Else
            If e.Item.Text <> "" Then
                ...
                Page.Header.Title = "Consulta de Notificaciones Operaciones Cevaldom -> " & e.Item.Text
    ...
    Protected Sub CargarInformacionFiltro(cNombreConsultaUsuario As String)
        Dim ds As DataSet = oper.ExDataSet(String.Format("SELECT idConsulta,Filtro, EstruturaGrid FROM  [vFiltrosConsultas]  where NombreConsultaUsuario='{0}' AND IdUsuario='{1}' AND NombreConsulta='ConsultaOperacionesNotificacionesCevaldom'", cNombreConsultaUsuario, txtIdUsuario.Text))
    ...
    Protected Sub LlenarInformacionListaFiltros()
        ...
        Dim ds As DataSet = oper.ExDataSet(String.Format("SELECT NombreConsultaUsuario FROM  [vFiltrosConsultas]  where IdUsuario='{0}' AND NombreConsulta='ConsultaOperacionesNotificacionesCevaldom'", txtIdUsuario.Text))
        Dim MyRow As DataRow
        Dim cStringFiltro As String = ""
    ...
    Function ReenviaOperacionCevaldom(NoOperacion As Decimal) As String
        Dim Sql As String = "SIOPEL_INTERFACE_DB.DBO.P_GEN_CEVALDOM_OPER_REENVIAR " + NoOperacion.ToString()
        ...
        Try
            ...
            'Cmd.Parameters.Add("@NoOperacion", SqlDbType.BigInt).Value = NoOperacion
            ' IIf(Me.txtNombre.Text.Length > 0, Me.txtNombre.Text, DBNull.Value)
            Cmd.ExecuteNonQuery()
            Return ""
        Catch ex As Exception
            Return ex.Message
    ...
    Protected Sub RadGrid1_ItemCommand(sender As Object, e As GridCommandEventArgs) Handles RadGrid1.ItemCommand
        ...
        If e.CommandName = "Reenviar" Then
            ...
            Dim value As String = item("REFERENCIA").Text
            ReenviaOperacionCevaldom(value)
        End If
