...
Partial Class ConsultaOperacionesOTCCevaldom
    Inherits Page
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If txtNombreConsultaUsuario.Text <> "" Then
            Page.Header.Title = "Consulta de Operaciones OTC Cevaldom -> " & txtNombreConsultaUsuario.Text
        End If
        oper.GenerarSeguimientoActividadUsuario(Session("IdUsuario"), "Ingreso a Consulta: Operaciones OTC Cevaldom", "Consulta de Operaciones OTC Cevaldom", "")
    End Sub
    ...
    Protected Sub RadToolBar1_ButtonClick(sender As Object, e As RadToolBarEventArgs) Handles RadToolBar1.ButtonClick, RadToolBar2.ButtonClick
        If e.Item.Value <> "" Then
            ...
            If e.Item.Value = 0 Then 'Mover
                ...
                InjectScriptLabelImprimir.Text = "<script>ventanaSecundaria('../Consultas/ConsultaOperacionesOTCCevaldom.aspx','1000','600')</" + "script>"
            ...
            ElseIf e.Item.Value = 2 Then 'Guardar
                ...
                MyWindow.NavigateUrl = String.Format("NombreConsultaUsuario.aspx?ID={0}&NC=ConsultaOperacionesOTCCevaldom&U={1}&NCU={2}&F={3}", txtIdConsulta.Text, txtIdUsuario.Text, txtNombreConsultaUsuario.Text, RadFilter1.SaveSettings())
        ...
        Else
            If e.Item.Text <> "" Then
                ...
                Page.Header.Title = "Consulta de Operaciones OTC Cevaldom -> " & e.Item.Text
    ...
    Protected Sub CargarInformacionFiltro(cNombreConsultaUsuario As String)
        ...
        Dim ds As DataSet = oper.ExDataSet(String.Format("SELECT idConsulta, Filtro,EstruturaGrid FROM  [vFiltrosConsultas]  where NombreConsultaUsuario='{0}' AND IdUsuario='{1}' AND NombreConsulta='ConsultaOperacionesOTCCevaldom'", cNombreConsultaUsuario, txtIdUsuario.Text))
    ...
    Protected Sub LlenarInformacionListaFiltros()
        ...
        Dim ds As DataSet = oper.ExDataSet(String.Format("SELECT NombreConsultaUsuario FROM  [vFiltrosConsultas]  where IdUsuario='{0}' AND NombreConsulta='ConsultaOperacionesOTCCevaldom'", txtIdUsuario.Text))
        Dim MyRow As DataRow
        Dim cStringFiltro As String = ""
    ...
    Protected Sub LlenarInformacionListaFiltros1()
        Dim ds As DataSet = oper.ExDataSet(String.Format("SELECT NombreConsultaUsuario FROM  [FiltrosConsultas]  where IdUsuario='{0}' AND NombreConsulta='ConsultaOperacionesOTCCevaldom' ORDER BY idConsulta ASC", txtIdUsuario.Text))
        Dim MyRow As DataRow
        Dim cStringFiltro As String = ""
    ...
    Sub DataRefresh()
        SqlvConsultaOperacionesOTCCevaldom.SelectCommandType = SqlDataSourceCommandType.StoredProcedure
        ...
        SqlvConsultaOperacionesOTCCevaldom.SelectParameters("SqlWhere").DefaultValue = cStringWhere
        SqlvConsultaOperacionesOTCCevaldom.SelectCommand = "SP_ConsultaOperacionesOTCCevaldom"
