...
Partial Class Herramientas_ConsultaOrdenesFirme
    Inherits Page
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Dim FechaOperacion As Date = oper.ExecuteScalar("Select Top(1) FechaPostura from [vOrdenesEnFirmeTemporal] order by FechaPostura desc")
            cStringWhere = String.Format("[FechaPostura]='{0}'", FechaOperacion)
            ...
            If Not IsPostBack Then
                editor.FieldName = "Fecha de Postura"
                editor.DataType = GetType(Date)

                Dim expr As New RadFilterEqualToFilterExpression(Of Date)("FechaPostura")
            ...
            DataRefresh()
            oper.GenerarSeguimientoActividadUsuario(Session("IdUsuario"), "Ingreso a Consulta: Operaciones", "Consulta de Ordenes en Firme", "")
        ...
        End If
        oper.GenerarSeguimientoActividadUsuario(Session("IdUsuario"), "Ingreso a Consulta: Operaciones", "Consulta Cevaldom", "")
    ...
    Protected Sub RadToolBar1_ButtonClick(sender As Object, e As RadToolBarEventArgs) Handles RadToolBar1.ButtonClick, RadToolBar2.ButtonClick
        If e.Item.Value <> "" Then
            ...
            If e.Item.Value = 0 Then 'Mover
                ...
                InjectScriptLabelImprimir.Text = "<script>ventanaSecundaria('../Consultas/ConsultaOrdenesFirme.aspx','1000','600')</" + "script>"
            ...
            ElseIf e.Item.Value = 2 Then 'Guardar
                ...
                MyWindow.NavigateUrl = String.Format("NombreConsultaUsuario.aspx?ID={0}&NC=ConsultaOrdenesFirme&U={1}&NCU={2}", txtIdConsulta.Text, txtIdUsuario.Text, txtNombreConsultaUsuario.Text) '& "&F=" & RadFilter1.SaveSettings()
        ...
        Else
            ...
            If e.Item.Text <> "" Then
                ...
                Page.Header.Title = "Consulta Ordenes en Firme -> " & e.Item.Text
    ...
    Protected Sub CargarInformacionFiltro(cNombreConsultaUsuario As String)
        ...
        Dim ds As DataSet = oper.ExDataSet(String.Format("SELECT idConsulta, Filtro, EstruturaGrid FROM  [vFiltrosConsultas]  where NombreConsultaUsuario='{0}' AND IdUsuario='{1}' AND NombreConsulta='ConsultaOrdenesFirme'", cNombreConsultaUsuario, txtIdUsuario.Text))
    ...
    Protected Sub LlenarInformacionListaFiltros()
        ...
        Dim ds As DataSet = oper.ExDataSet(String.Format("SELECT NombreConsultaUsuario FROM  [vFiltrosConsultas]  where IdUsuario='{0}' AND NombreConsulta='ConsultaOrdenesFirme'", txtIdUsuario.Text))
        Dim MyRow As DataRow
        Dim cStringFiltro As String = ""
    ...
    Protected Sub LlenarInformacionListaFiltros1()
        ...
        Dim ds As DataSet = oper.ExDataSet(String.Format("SELECT NombreConsultaUsuario FROM  [FiltrosConsultas]  where IdUsuario='{0}' AND NombreConsulta='ConsultaOrdenesFirme' ORDER BY idConsulta ASC", txtIdUsuario.Text))
        Dim MyRow As DataRow
        Dim cStringFiltro As String = ""
    ...
    Protected Sub RadFilter1_ApplyExpressions(sender As Object, e As RadFilterApplyExpressionsEventArgs) Handles RadFilter1.ApplyExpressions
        ...
        End Try
    End Sub
    Protected Sub SqlvConsultaOrdenesFirme_Selecting(ByVal sender As Object, ByVal e As SqlDataSourceSelectingEventArgs) Handles SqlvConsultaOrdenesFirme.Selecting
        e.Command.CommandTimeout = 0
    End Sub
    Sub DataRefresh()
        ' SqlvConsultaOrdenesFirme.CommandTimeout = 0
        '  SqlvConsultaOrdenesFirme.
        SqlvConsultaOrdenesFirme.SelectCommandType = SqlDataSourceCommandType.StoredProcedure
        ...
        SqlvConsultaOrdenesFirme.SelectParameters("SqlWhere").DefaultValue = cStringWhere
        SqlvConsultaOrdenesFirme.SelectCommand = "SP_ConsultaOrdenesFirme"
    End Sub
    ' Eventos del RADGRID
