...
Partial Class ConsultaParticipantesMercado
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Dim FechadeOperacion As Date = oper.ExecuteScalar("Select Top(1) FechaConstitucion from [vPuestoBolsa] order by FechaConstitucion desc")
            cStringWhere = "[FechaConstitucion]='" & FechadeOperacion & "'"
            '--------------------------------------------------------------------------------------
            ...
            ' Agregar columna(s) al GRID

            Dim ds As New DataSet
            ds = oper.ExDataSet("select Descripcion from mecanismosnegociacion order by descripcion")

            For Each table As DataTable In ds.Tables

                For Each dr As DataRow In table.Rows
                    Dim cNombre As String = dr("Descripcion").ToString()

                    Dim BoundColumn As GridBoundColumn
                    BoundColumn = New GridBoundColumn()
                    RadGrid1.MasterTableView.Columns.Add(BoundColumn)
                    BoundColumn.DataField = cNombre
                    BoundColumn.HeaderText = cNombre
                    BoundColumn.UniqueName = cNombre
        ...
        If txtNombreConsultaUsuario.Text <> "" Then
            Page.Header.Title = "Consulta de Participantes Mercado -> " & txtNombreConsultaUsuario.Text
        End If
        oper.GenerarSeguimientoActividadUsuario(Session("IdUsuario"), "Ingreso a Consulta: Participantes Mercado", "Consulta Participantes Mercado", "")
    End Sub
    ...
    Protected Sub RadToolBar1_ButtonClick(sender As Object, e As RadToolBarEventArgs) Handles RadToolBar1.ButtonClick, RadToolBar2.ButtonClick
        If e.Item.Value <> "" Then
            ...
            If e.Item.Value = 0 Then 'Mover
                ...
                InjectScriptLabelImprimir.Text = "<script>ventanaSecundaria('../Consultas/ConsultaParticipantesMercado.aspx','1000','600')</" + "script>"
            ...
            ElseIf e.Item.Value = 2 Then 'Guardar
                ...
                MyWindow.NavigateUrl = "NombreConsultaUsuario.aspx?ID=" & txtIdConsulta.Text & "&NC=ConsultaParticipantesMercadoV&U=" & txtIdUsuario.Text & "&NCU=" & txtNombreConsultaUsuario.Text & "&F=" & RadFilter1.SaveSettings()
        ...
        Else
            ...
            If e.Item.Text <> "" Then
                ...
                Page.Header.Title = "Consulta Participante Mercado -> " & e.Item.Text
    ...
    Protected Sub CargarInformacionFiltro(cNombreConsultaUsuario As String)
        ...
        Dim ds As DataSet = oper.ExDataSet("SELECT idConsulta, Filtro,EstruturaGrid FROM  [vFiltrosConsultas]  where NombreConsultaUsuario='" & cNombreConsultaUsuario & "' AND IdUsuario='" & txtIdUsuario.Text & "' AND NombreConsulta='ConsultaParticipantesMercado'")
    ...
    Protected Sub LlenarInformacionListaFiltros()
        ...
        Dim ds As DataSet = oper.ExDataSet("SELECT NombreConsultaUsuario FROM  [vFiltrosConsultas]  where IdUsuario='" & txtIdUsuario.Text & "' AND NombreConsulta='ConsultaParticipantesMercado'")
        Dim MyRow As DataRow
        Dim cStringFiltro As String = ""
    ...
    Protected Sub LlenarInformacionListaFiltros1()
        ...
        Dim ds As DataSet = oper.ExDataSet("SELECT NombreConsultaUsuario FROM  [FiltrosConsultas]  where IdUsuario='" & txtIdUsuario.Text & "' AND NombreConsulta='ConsultaParticipantesMercado' ORDER BY idConsulta ASC")
        Dim MyRow As DataRow
        Dim cStringFiltro As String = ""
    ...
    Sub DataRefresh()
        ...
        SqlvParticipantesMercado.SelectCommandType = SqlDataSourceCommandType.StoredProcedure
        ...
        End If
        SqlvParticipantesMercado.SelectParameters("SqlWhere").DefaultValue = cStringWhere
        SqlvParticipantesMercado.SelectCommand = "[SP_ConsultaParticipantesMercado]"
    End Sub
