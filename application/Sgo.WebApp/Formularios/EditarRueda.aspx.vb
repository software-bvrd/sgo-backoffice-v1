...
Partial Class Formularios_EditarRueda
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ViewState("Code") = Request.QueryString("RuedaID")
        End If
    ...
    Sub Nuevo()
        If ViewState("EsNuevo") = True Then
            ...
            oper.ExecuteNonQuery("INSERT INTO Rueda (Nombre,CodRueda, Activo) VALUES ('" & txtNombre.Text & "','" & txtRuedaCodigo.Text & "','" & Check & "')")
        End If
    End Sub

    Sub Editar()
        Dim ds As DataSet = oper.ExDataSet("SELECT * FROM  [Rueda]  where RuedaID='" & CInt(ViewState("Code")) & "'")
        Dim MyRow As DataRow
        For Each MyRow In ds.Tables(0).Rows
            If Not IsDBNull(MyRow.Item("Nombre")) Then Me.txtNombre.Text = Trim(MyRow.Item("Nombre"))
            If Not IsDBNull(MyRow.Item("CodRueda")) Then Me.txtRuedaCodigo.Text = Trim(MyRow.Item("CodRueda"))
    ...
    Sub Actualizar()
        Dim Check As String
        Check = CheckBox1.Checked
        oper.ExecuteNonQuery("Update [Rueda] set  Nombre='" & txtNombre.Text & "', CodRueda = '" & txtRuedaCodigo.Text & "', Activo='" & Check & "'  where RuedaID='" & CInt(ViewState("Code")) & "'")
        ViewState("EsNuevo") = False
    End Sub

    Sub Borrar()

        If oper.ExecuteNonQuery("delete from dbo.Rueda where RuedaID='" & ViewState("Code") & "'") Then
